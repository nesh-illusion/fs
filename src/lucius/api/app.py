...
from lucius.config.protocols import ProtocolRegistry
from lucius.config.settings import AppSettings
from lucius.ledger.memory_store import (
    MemoryJobIndexStore,
...
from lucius.ledger.models import Job, JobIdempotency, JobIndex, OutboxEntry, Step
from lucius.ledger.table_storage import (
...
from lucius.validation.idempotency import idempotency_hash
...
def _partition_for_tenant(tenant_id: str, buckets: int = 16) -> str:
    return str(zlib.crc32(tenant_id.encode("utf-8")) % buckets)
...
def create_app() -> FastAPI:
    ...
    async def create_command(envelope: Dict[str, Any]):
        ...
        job = Job(
            ...
            mode=envelope.get("mode", "DEFAULT"),
            state="QUEUED",
        ...
        outbox = OutboxEntry(
            ...
            lease_id=lease_id,
            state="PENDING",
            topic=f"global-bus-p{_partition_for_tenant(envelope['tenant_id'])}",
            partition=_partition_for_tenant(envelope["tenant_id"]),
            payload={
                "jobId": job_id,
                "tenant_id": envelope["tenant_id"],
